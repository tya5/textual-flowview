...
| :- | :- | :- | :- |
| `FlowPresenter.present(entry, width)` | **yes** (`async def`) | per entry, per revision, per width — **the hot one**; the revision also bumps when the entry folds or gains/loses a child | `await`ing here frees the loop; a pure-CPU body does not |
