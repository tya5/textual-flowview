...
@pytest.mark.asyncio
async def test_declared_selected_highlight_still_paints() -> None:
    # Regression: a declared selected highlight still shows (the fix must not
    # break the normal, declared case).
    css = """
    FlowView { height: 1fr; }
    FlowView > .flowview--selected { background: #0000ff; }
    """
